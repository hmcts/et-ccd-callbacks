...
class FixMultipleTransferAPICallbackHandlerTest {
    ...
    private FixMultipleTransferAPICallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new FixMultipleTransferAPICallbackHandler(
            caseDetailsConverter,
            verifyTokenService,
            multipleAmendService,
            fixMultipleCaseApiService
        );
    }

    @Test
    void aboutToSubmitShouldNotFixWhenTokenInvalid() {
        ...
        verify(multipleAmendService, never()).bulkAmendMultipleLogic(any(), any(), any());
        verify(fixMultipleCaseApiService, never()).fixMultipleCase(any(), any(), any());
    }

    @Test
    void aboutToSubmitShouldAmendAndFixWhenTokenValid() {
        MultipleDetails multipleDetails = multipleDetails();
        stubMultipleConverter(multipleDetails);
        ...
        verify(multipleAmendService).bulkAmendMultipleLogic(eq(null), eq(multipleDetails), any());
        verify(fixMultipleCaseApiService).fixMultipleCase(eq(null), eq(multipleDetails), any());
    }
