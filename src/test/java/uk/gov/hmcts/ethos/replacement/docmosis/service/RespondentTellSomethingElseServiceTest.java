...
class RespondentTellSomethingElseServiceTest {
    ...
    @ParameterizedTest
    @MethodSource
    void resTseSetVariableContentReturnString(String selectedApplication, String expected) {
        CaseData caseData = CaseDataBuilder.builder().build();
        caseData.setResTseSelectApplication(selectedApplication);
        CaseDetails caseDetails = new CaseDetails();
        caseDetails.setCaseData(caseData);
        caseDetails.setCaseId(CASE_ID);
        assertThat(respondentTellSomethingElseService.resTseSetVariableContent(caseData))
                .isEqualTo(expected);
    }

    private static Stream<Arguments> resTseSetVariableContentReturnString() {
        return Stream.of(
                Arguments.of(SELECTED_APP_AMEND_RESPONSE,
                        VARIABLE_CONTENT_AMEND_RESPONSE),
                Arguments.of(SELECTED_APP_CHANGE_PERSONAL_DETAILS,
                        VARIABLE_CONTENT_CHANGE_PERSONAL_DETAILS),
                Arguments.of(SELECTED_APP_CLAIMANT_NOT_COMPLIED,
                        VARIABLE_CONTENT_CLAIMANT_NOT_COMPLIED),
                Arguments.of(SELECTED_APP_CONSIDER_A_DECISION_AFRESH,
                        VARIABLE_CONTENT_CONSIDER_A_DECISION_AFRESH),
                Arguments.of(SELECTED_APP_CONTACT_THE_TRIBUNAL,
                        VARIABLE_CONTENT_CONTACT_THE_TRIBUNAL),
                Arguments.of(SELECTED_APP_ORDER_OTHER_PARTY,
                        VARIABLE_CONTENT_ORDER_OTHER_PARTY),
                Arguments.of(SELECTED_APP_ORDER_A_WITNESS_TO_ATTEND_TO_GIVE_EVIDENCE,
                        VARIABLE_CONTENT_ORDER_A_WITNESS_TO_ATTEND_TO_GIVE_EVIDENCE),
                Arguments.of(SELECTED_APP_POSTPONE_A_HEARING,
                        VARIABLE_CONTENT_POSTPONE_A_HEARING),
                Arguments.of(SELECTED_APP_RECONSIDER_JUDGEMENT,
                        VARIABLE_CONTENT_RECONSIDER_JUDGEMENT),
                Arguments.of(SELECTED_APP_RESTRICT_PUBLICITY,
                        VARIABLE_CONTENT_RESTRICT_PUBLICITY),
                Arguments.of(SELECTED_APP_STRIKE_OUT_ALL_OR_PART_OF_A_CLAIM,
                        VARIABLE_CONTENT_STRIKE_OUT_ALL_OR_PART_OF_A_CLAIM),
                Arguments.of(SELECTED_APP_VARY_OR_REVOKE_AN_ORDER,
                        VARIABLE_CONTENT_VARY_OR_REVOKE_AN_ORDER));
    }
