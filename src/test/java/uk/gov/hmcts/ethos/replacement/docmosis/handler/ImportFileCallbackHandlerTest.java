...
class ImportFileCallbackHandlerTest {
    ...
    private ImportFileCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new ImportFileCallbackHandler(caseDetailsConverter, verifyTokenService, multipleUploadService);
    }

    @Test
    void aboutToSubmitShouldNotImportWhenTokenInvalid() {
        ...
        verify(multipleUploadService, never()).bulkUploadLogic(any(), any(), any());
    }

    @Test
    void aboutToSubmitShouldImportWhenTokenValid() {
        MultipleDetails multipleDetails = multipleDetails();
        stubMultipleConverter(multipleDetails);
        ...
        verify(multipleUploadService).bulkUploadLogic(eq(null), eq(multipleDetails), any());
    }
