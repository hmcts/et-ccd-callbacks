...
class CaseTransferECMCallbackHandlerTest {
    ...
    private CaseTransferECMCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new CaseTransferECMCallbackHandler(
            caseDetailsConverter,
            verifyTokenService,
            caseTransferToEcmService
        );
    }
    ...
    void aboutToSubmitShouldNotTransferWhenTokenInvalid() {
        ...
        verify(caseTransferToEcmService, never()).createCaseTransferToEcm(any(), any());
    }
    ...
    void aboutToSubmitShouldTransferWhenTokenValid() {
        ...
        when(caseTransferToEcmService.createCaseTransferToEcm(any(), any())).thenReturn(List.of());

        handler.aboutToSubmit(callbackCaseDetails());

        verify(caseTransferToEcmService).createCaseTransferToEcm(any(), any());
    }
