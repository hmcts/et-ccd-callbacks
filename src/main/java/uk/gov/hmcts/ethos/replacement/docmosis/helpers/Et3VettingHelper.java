...
public class Et3VettingHelper {
    public static String getEt3Dates(CaseData caseData) {
        ...
        stringBuilder.append(String.format("|ET3 due| %s|\r\n", formatDate(findEt3Due(caseData.getClaimServedDate()))));
    ...
    private static String findEt3Due(String claimServedDate) {
        if (isNullOrEmpty(claimServedDate)) {
            return "Cannot find ET3 due date";
        }

        LocalDate localDate = LocalDate.parse(claimServedDate).plusDays(29);
    ...
    private static String formatDate(String date) {
        if (isNullOrEmpty(date)) {
            return "Claim not served";
        } else {
            return UtilHelper.listingFormatLocalDate(date);
        }
    }
    ...
    private static String findEt3Received(CaseData caseData) {
        ...
        for (RespondentSumTypeItem respondentSumTypeItem : respondentCollection) {
            ...
            if (respondent.getResponseReceived().equals(YES)) {
                ...
                return formatDate(respondent.getResponseReceivedDate());
            }
        }
