...
public class SupportTaskEventService {
    ...
    public void triggerCloseReviewSupportTasks(CaseDetails caseDetails) {
        ...
        SupportTaskState completedTaskState = caseDetails.getCaseData().getSupportTaskState();
        try {
            ...
            CaseData latestCaseData = latestCaseDetails.getCaseData();
            if (latestCaseData.getSupportTaskState() == null) {
                latestCaseData.setSupportTaskState(new SupportTaskState());
            }
            markCompletedReviewTasksNotRequired(latestCaseData.getSupportTaskState(), completedTaskState);
    ...
    private static void markCompletedReviewTasksNotRequired(SupportTaskState latestTaskState,
                                                             SupportTaskState completedTaskState) {
        if (completedTaskState == null) {
            return;
        }
        if (isTaskNotCreated(completedTaskState.getAdminTaskCreated())) {
            latestTaskState.setAdminTaskRequired(NO);
        }
        if (isTaskNotCreated(completedTaskState.getLegalOfficerTaskCreated())) {
            latestTaskState.setLegalOfficerTaskRequired(NO);
        }
        if (isTaskNotCreated(completedTaskState.getJudgeTaskCreated())) {
            latestTaskState.setJudgeTaskRequired(NO);
        }
    }

    private static boolean isTaskNotCreated(String taskCreated) {
        return NO.equalsIgnoreCase(taskCreated) || "false".equalsIgnoreCase(taskCreated);
    }
}
