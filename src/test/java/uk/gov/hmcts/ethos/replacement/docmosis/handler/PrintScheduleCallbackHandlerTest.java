...
class PrintScheduleCallbackHandlerTest {
    ...
    private PrintScheduleCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new PrintScheduleCallbackHandler(
            caseDetailsConverter,
            verifyTokenService,
            multipleScheduleService,
            multipleDocGenerationCallbackService
        );
    }

    @Test
    void aboutToSubmitShouldNotPrintScheduleWhenTokenInvalid() {
        ...
        verify(multipleScheduleService, never()).bulkScheduleLogic(any(), any(), any());
    }

    @Test
    void aboutToSubmitShouldPrintScheduleWhenTokenValid() {
        MultipleDetails multipleDetails = multipleDetails();
        stubMultipleConverter(multipleDetails);
        ...
        when(multipleScheduleService.bulkScheduleLogic(eq(null), eq(multipleDetails), any()))
            .thenReturn(DocumentInfo.builder().markUp("schedule").build());

        handler.aboutToSubmit(callbackCaseDetails());

        verify(multipleScheduleService).bulkScheduleLogic(eq(null), eq(multipleDetails), any());
    }
