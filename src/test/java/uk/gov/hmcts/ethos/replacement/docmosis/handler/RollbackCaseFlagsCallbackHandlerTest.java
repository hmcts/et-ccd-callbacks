...
class RollbackCaseFlagsCallbackHandlerTest {
    ...
    private RollbackCaseFlagsCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new RollbackCaseFlagsCallbackHandler(caseDetailsConverter, verifyTokenService, caseFlagsService);
    }

    @Test
    void aboutToSubmitShouldNotRollbackFlagsWhenTokenInvalid() {
        ...
        verify(caseFlagsService, never()).rollbackCaseFlags(caseData);
    }

    @Test
    void aboutToSubmitShouldRollbackFlagsWhenTokenValid() {
        CaseData caseData = new CaseData();
        stubConverter(caseData);
        ...
        verify(caseFlagsService).rollbackCaseFlags(caseData);
    }
