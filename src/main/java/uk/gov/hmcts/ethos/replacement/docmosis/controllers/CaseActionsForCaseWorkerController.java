...
public class CaseActionsForCaseWorkerController {
    ...
    @Value("${rd_professional.api.url:notfound}")
    private String rdProAPIurl;
    ...
    public ResponseEntity<CCDCallbackResponse> amendRespondentRepresentative(
            @RequestBody CCDRequest ccdRequest,
            @RequestHeader(value = "Authorization") String userToken) {
        log.info("AMEND RESPONDENT REPRESENTATIVE ---> "
                + LOG_MESSAGE
        ...
        // DEBUG
        Organisation org = caseData.getRepCollection().get(0).getValue().getRespondentOrganisation();
        if (org.getOrganisationAddress() == null) {
            log.info("ORGANISATION ADDRESS MISSING");
        }
        // DEBUG
