...
class PreAcceptMultipleCallbackHandlerTest {
    ...
    private PreAcceptMultipleCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new PreAcceptMultipleCallbackHandler(
            caseDetailsConverter,
            verifyTokenService,
            multiplePreAcceptService
        );
    }

    @Test
    void aboutToSubmitShouldNotPreAcceptWhenTokenInvalid() {
        ...
        verify(multiplePreAcceptService, never()).bulkPreAcceptLogic(any(), any(), any());
    }

    @Test
    void aboutToSubmitShouldPreAcceptWhenTokenValid() {
        MultipleDetails multipleDetails = multipleDetails();
        stubMultipleConverter(multipleDetails);
        ...
        verify(multiplePreAcceptService).bulkPreAcceptLogic(eq(null), eq(multipleDetails), any());
    }
