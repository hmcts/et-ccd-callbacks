...
public class UpdateReferralMultiplesController {
    ...
    public ResponseEntity<MultipleCallbackResponse> initHearingDetailsForUpdateReferral(
        ...
        if (caseData.getLeadCaseId() != null) {
            leadCase = caseLookupService.getCaseDataAsAdmin(caseTypeId, caseData.getLeadCaseId());
            caseData.setHearingAndReferralDetails(ReferralHelper.populateHearingReferralDetails(caseData, leadCase));
        } else {
            leadCase = new ObjectMapper().convertValue(request.getCaseDetails().getCaseData(), CaseData.class);
        }

        if (ReferralHelper.isValidReferralStatus(leadCase)) {
            ReferralHelper.populateUpdateReferralDetails(leadCase);
            caseData.setHearingAndReferralDetails(
                    ReferralHelper.populateHearingReferralDetails(caseData, leadCase));
        } else {
