...
class SubmitRespondentTseCallbackHandlerTest {
    ...
    private SubmitRespondentTseCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new SubmitRespondentTseCallbackHandler(
    ...
    @Test
    void aboutToSubmitShouldCreateRespondentApplicationWhenPresent() {
        ...
        verify(tseService).createApplication(caseData, RESPONDENT_TITLE);
        verify(tseService).removeStoredRespondentApplication(caseData);
        verify(tseService).clearApplicationData(caseData);
        verify(caseManagementForCaseWorkerService).setNextListedDate(caseData);
    }

    @Test
    void aboutToSubmitShouldSkipApplicationCreationWhenNoRespondentTse() {
        CaseData caseData = new CaseData();
        stubConverter(caseData);
