...
class UploadHearingDocumentsCallbackHandlerTest {
    ...
    private UploadHearingDocumentsCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new UploadHearingDocumentsCallbackHandler(caseDetailsConverter, hearingDocumentsService);
    }

    @Test
    void aboutToSubmitShouldDelegateToHearingDocumentsService() {
        CaseData caseData = new CaseData();
        stubConverter(caseData);

        handler.aboutToSubmit(callbackCaseDetails());

        verify(hearingDocumentsService).addDocumentToHearingDocuments(caseData, null);
    }
