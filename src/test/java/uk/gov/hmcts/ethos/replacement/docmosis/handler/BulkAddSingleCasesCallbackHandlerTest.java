...
class BulkAddSingleCasesCallbackHandlerTest {
    ...
    private BulkAddSingleCasesCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new BulkAddSingleCasesCallbackHandler(
            caseDetailsConverter,
            bulkAddSinglesService,
            verifyTokenService
        );
    }

    @Test
    void aboutToSubmitShouldNotExecuteBulkAddWhenTokenInvalid() {
        ...
        verify(bulkAddSinglesService, never()).execute(any(), any());
    }

    @Test
    void aboutToSubmitShouldExecuteBulkAddWhenTokenValid() {
        MultipleDetails multipleDetails = multipleDetails();
        stubMultipleConverter(multipleDetails);
        ...
        when(bulkAddSinglesService.execute(multipleDetails, null)).thenReturn(List.of());

        handler.aboutToSubmit(callbackCaseDetails());

        verify(bulkAddSinglesService).execute(multipleDetails, null);
    }
