...
public final class NoPositionChangeReportData extends ListingData {
    ...
    public StringBuilder toReportObjectString() throws JsonProcessingException {
        ...
        sb.append(REPORT_OFFICE).append(reportSummary.getOffice()).append(NEW_LINE).append(REPORT_DATE)
                .append(UtilHelper.listingFormatLocalDate(getReportDate())).append(NEW_LINE)
                .append(TOTAL_CASES).append(StringUtils.defaultIfEmpty(reportSummary.getTotalCases(), "0"))
                .append(NEW_LINE).append(TOTAL_SINGLE)
                .append(StringUtils.defaultIfEmpty(reportSummary.getTotalSingleCases(), "0")).append(NEW_LINE)
                .append(TOTAL_MULTIPLE)
                .append(StringUtils.defaultIfEmpty(reportSummary.getTotalMultipleCases(), "0"))
                .append(NEW_LINE);
