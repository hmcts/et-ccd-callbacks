...
class CreateMultipleCallbackHandlerTest {
    ...
    private CreateMultipleCallbackHandler handler;

    @BeforeEach
    void setUp() {
        handler = new CreateMultipleCallbackHandler(caseDetailsConverter, verifyTokenService, multipleCreationService);
    }

    @Test
    void aboutToSubmitShouldNotCreateWhenTokenInvalid() throws IOException {
        ...
        verify(multipleCreationService, never()).bulkCreationLogic(any(), any(), any());
    }

    @Test
    void aboutToSubmitShouldCreateWhenTokenValid() throws IOException {
        MultipleDetails multipleDetails = multipleDetails();
        stubMultipleConverter(multipleDetails);
        ...
        verify(multipleCreationService).bulkCreationLogic(eq(null), eq(multipleDetails), any());
    }

    @Test
    void aboutToSubmitShouldWrapIOExceptionFromCreateMultiple() throws IOException {
        MultipleDetails multipleDetails = multipleDetails();
        stubMultipleConverter(multipleDetails);
        when(verifyTokenService.verifyTokenSignature(null)).thenReturn(true);
        doThrow(new IOException("create failed")).when(multipleCreationService)
            .bulkCreationLogic(any(), any(), any());

        assertThatThrownBy(() -> handler.aboutToSubmit(callbackCaseDetails()))
            .isInstanceOf(IllegalStateException.class)
            .hasMessageContaining("Failed to create multiple");
    }
