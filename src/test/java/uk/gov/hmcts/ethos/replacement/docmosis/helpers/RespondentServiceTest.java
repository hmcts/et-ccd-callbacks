...
class RespondentServiceTest {
    ...
    void shouldReturnRespondentRepOrganisations() {
        Map<String, Organisation> respondentOrganisations = respondentService.getRespondentOrganisations(caseData);
    ...
    void shouldReturnOrganisationOfRep() {
        Organisation orgFromRep = respondentService.getOrgFromRep(RESPONDENT_ID_THREE, caseData.getRepCollection());
        assertThat(orgFromRep).isEqualTo(org1);
    }

    @Test
    void shouldReturnIndexOfRep() {
        assertThat(respondentService.getIndexOfRep(respondentSumTypeItem1,
            caseData.getRepCollection())).isEqualTo(2);
        assertThat(respondentService.getIndexOfRep(respondentSumTypeItem2,
            caseData.getRepCollection())).isEqualTo(1);
        assertThat(respondentService.getIndexOfRep(respondentSumTypeItem3,
            caseData.getRepCollection())).isEqualTo(0);
        assertThat(respondentService.getIndexOfRep(unknownRespondentSumTypeItem,
            caseData.getRepCollection())).isEqualTo(-1);
    }
    ...
    void shouldReturnRespondentRep() {
        Optional<RepresentedTypeRItem> respondentRep =
            respondentService.getRespondentRep(respondentSumTypeItem1, caseData.getRepCollection());
    ...
    void shouldCreateChangeRequest() {
        ChangeOrganisationRequest changeRequest =
            respondentService.createChangeRequest(org1, org2, SolicitorRole.SOLICITORD);
    ...
    void generateNewRepDetails() {
        ...
        RepresentedTypeR representedTypeR =
            respondentService.generateNewRepDetails(expectedChangeRequest(), Optional.of(userDetails),
