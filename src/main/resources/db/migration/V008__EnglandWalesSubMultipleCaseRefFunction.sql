CREATE
    OR REPLACE FUNCTION fn_EnglandWalesEthosSubMultipleCaseRefGen(p_multref INT, p_numofcases INT)
    RETURNS VARCHAR(100) AS
$$

-- =============================================
-- TEST :		SELECT fn_EnglandWalesEthosSubMultipleCaseRefGen(243, 1);
--
-- Create date: 14-APR-2020
-- Description:	Function to generate Ethos case reference numbers for submultiple cases for England/Wales
...
BEGIN
    SELECT submultref
    INTO c_submultref
    FROM submultiple_reference_englandwales
    WHERE multref = p_multref FOR UPDATE;

    IF c_submultref IS NULL THEN
    ...
    ELSE
        UPDATE submultiple_reference_englandwales
